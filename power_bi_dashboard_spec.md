...
Needed for cycle-time-by-activity and queue-wait measures. The event log includes an `event_seq` column (a per-row global counter reflecting true logging order), added specifically to break ties when two events share an identical timestamp — this happens whenever a resource is immediately available (e.g. zero manager-queue wait), since both the "submitted" and "started" events get logged at the same instant. Comparing on `timestamp` alone is ambiguous in that case; DAX has no inherent concept of row order the way a dataframe does, so without `event_seq` it can silently skip past the tied pair and land on an earlier, unrelated event instead — caught by cross-checking dashboard rows against the raw log directly.

Add as calculated columns, not measures (row context needed):
...
CALCULATE(
    ...
    FILTER(
        ...
        Events[claim_scenario_key] = EARLIER(Events[claim_scenario_key])
            && Events[event_seq] < EARLIER(Events[event_seq])
    )
)
